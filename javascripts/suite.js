(function() {
  ...
  fixture = {
    ...
    select: function(stage, scene) {
      ...
      if (typeof stage === 'number') {
        return Stagehand.$controls.find('> ul > li').eq(stage).find('li').eq(scene).find('a').click();
      } else {
        return this.stageControls(stage).eq(scene).find("a").click();
      }
    },
    stageControls: function(name) {
      return Stagehand.$controls.find("h2:contains('" + name + "')").next('ul').find("li");
    },
    stage: function(name) {
    ...
    init: function(selector) {
      ...
      Stagehand.init();
      this.$stagehand = $('#stagehand-controls');
  ...
  test('it detects siblings with `data-stage` attributes as one scene', function() {
    ...
    return equal(Stagehand.$controls.find('> ul > li').eq(1).find('li').length, 1, 'second scene control has one option');
  });
  ...
  test('support special `data-scene` attribute of `toggle`', function() {
    ...
    equal(Stagehand.$controls.find('li li').length, 4, 'scene control has 4 options');
    Stagehand.$controls.find('li li').first().shouldSay('toggle off').next().shouldSay('toggle on');
    $("[class^='actor']").shouldNotBe(':visible');
    fixture.select(0, 1);
